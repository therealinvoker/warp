...
- `execute`: `match self.pending.remove(id)` — `Failed` returns `DiffApplicationFailed`; `Prepared` calls `PersistDiffModel::handle(ctx)` -> `resolve_and_persist(diffs, reviewed.unwrap_or_default(), session_type, ..)`, returning `ActionExecution::new_async`. One path for both surfaces; no `CodeDiffView`, no executor-held completion channel (the persist model awaits its own writes internally), no per-caller resolution logic. Remove the `TODO(surface-agnostic-file-edit-execution)` left on the parent branch.
...
PHASE 1 preprocess: apply_diffs -> on_diffs_applied -> self.pending[id] = Prepared{..}
                                                       -> emits DiffsPrepared; the GUI block
                                                          feeds prepared diffs to its CodeDiffView
