...
//!
//! The view owns a [`HeadlessDiffStorageModel`] and registers it with the
...
use warp::tui_export::{
    ...
    HeadlessDiffStorage, HeadlessDiffStorageModel,
};
...
pub(super) struct TuiFileEditsView {
    ...
    storage: ModelHandle<HeadlessDiffStorageModel>,
}
...
impl TuiFileEditsView {
    pub(super) fn new(
        ...
        let storage = ctx.add_model(|ctx| {
            HeadlessDiffStorageModel::new(Vec::new(), DiffSessionType::Local, ctx)
        });
        ...
        executor.update(ctx, |executor, ctx| {
            executor.register_requested_edits(
                &action_id,
                Box::new(HeadlessDiffStorage(storage.clone())),
                ctx,
            );
        });
