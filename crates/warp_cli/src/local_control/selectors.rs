...
const INSTANCE_ID_ENV: &str = "WARPCTRL_INSTANCE_ID";

pub(super) fn instance_selector() -> Result<InstanceSelector, ControlError> {
    let instance_id = std::env::var(INSTANCE_ID_ENV).map_err(|_| {
        ControlError::new(
            ErrorCode::ExecutionContextNotAllowed,
            "warpctrl requests require the app-injected proof-issuing instance id",
        )
    })?;
    if instance_id.is_empty() {
        return Err(ControlError::new(
            ErrorCode::ExecutionContextNotAllowed,
            "the app-injected proof-issuing instance id cannot be empty",
        ));
    }
    Ok(InstanceSelector::Id(local_control::discovery::InstanceId(
        instance_id,
    )))
}
