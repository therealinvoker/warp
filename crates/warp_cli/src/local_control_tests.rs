...
#[test]
fn rejects_cross_instance_targeting_and_excluded_routes() {
    for args in [
        vec!["warpctrl", "tab", "create", "--instance", "inst_123"],
        vec!["warpctrl", "app", "ping", "--pid", "123"],
...
fn excluded_actions_are_not_allowlisted_catalog_entries() {
    for excluded in ["auth.api_key.set", "file.write"] {
        assert!(
            ActionKind::ALL
                .iter()
