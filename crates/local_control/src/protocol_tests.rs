...
#[test]
fn block_summary_is_content_free() {
    let summary = BlockSummary {
        block_id: "block_1".to_owned(),
        session_id: "session_1".to_owned(),
        index: 3,
    };
    let value = serde_json::to_value(summary).expect("block summary serializes");
    assert_eq!(
        value,
        serde_json::json!({
            "block_id": "block_1",
...
fn response_error_serializes_machine_code() {
    let response = ResponseEnvelope::error(
        Uuid::nil(),
        ControlError::new(ErrorCode::InvalidTerminalProof, "bad proof"),
    );
    let value = serde_json::to_value(&response).expect("response serializes");
    assert_eq!(value["response"]["status"], "error");
    assert_eq!(value["response"]["error"]["code"], "invalid_terminal_proof");
}
...
#[test]
fn catalog_has_exactly_76_retained_actions() {
    assert_eq!(ActionKind::ALL.len(), 76);
}
...
fn catalog_has_exact_confirmation_policy() {
    ...
    assert_eq!(ActionKind::ALL.len() - confirmed.len(), 73);
}
