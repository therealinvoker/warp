...
pub enum InitStepStatus {
    ...
    Ready(InitStepData),
    /// User initiated action, e.g. AI generating the project rules file
    Running,
                ...
                let has_agents_md = existing_files.iter().any(|p| {
                    p.file_name()
                        .map(|n| {
                            ...
                            name == "agents.md" || name == "bang.md" || name == "warp.md"
                        })
                        .unwrap_or(false)
                });

                if has_agents_md {
                    // Already has AGENTS.md, BANG.md, or WARP.md, mark as completed
                    me.set_step(
                        InitStepKind::ProjectScopedRules,
                        Some(InitStep::new_completed(
                            InitStepKind::ProjectScopedRules,
