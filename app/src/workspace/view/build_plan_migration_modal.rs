...
impl BuildPlanMigrationModal {
    ...
    fn render_left_panel(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        ...
        let intro_text = if is_business {
            "Your workspace has been updated to the new Bang Business Plan as the legacy Business plan is sunset."
        } else {
            "Your workspace has been updated to the Bang Build Plan as the legacy Pro, Turbo, and Lightspeed plans are sunset."
        };
        ...
        let pricing_header = Self::create_text(
            if is_business {
                ...
                "Bang Build is a primarily usage-based plan, starting at:"
            }
            .to_string(),
            font_family,
            14.,
